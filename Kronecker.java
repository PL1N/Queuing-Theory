...
public class Kronecker {
    ...
    public static double[][] readMatrix(Scanner in) {
        System.out.println("¬ведите размер квадртаной матрицы : ");
        int n = in.nextInt();
        double[][] arr = new double[n][n];
        for (int i = 0; i < n; ++i) {
            System.out.println("¬велдие элементы " + (i + 1) + " строки");
            for (int j = 0; j < n; ++j)
    ...
    public static double[] readVector(Scanner in) {
        System.out.println("¬ведите кол-во элементов вектора : ");
        int n = in.nextInt();
    ...
    public static double calcLambda() {
        ...
        if (lambda / mu >= 1) {
<<<<<<< HEAD
            System.out.println("јнин метод");
            ;
=======
            System.out.println("”словие не выполн€етс€");
    ...
    public static void main(final String[] args) {
        try {
           /* Scanner in = new Scanner(System.in);
            System.out.println("¬ведите матрицу D.txt :");
            D.txt = Kronecker.readMatrix(in);
            System.out.println("¬ведите матрицу D1 :");
            D1 = Kronecker.readMatrix(in);
            System.out.println("¬ведите матрицу S :");
            S = Kronecker.readMatrix(in);
            System.out.println("¬ведите вектор b :");
            b = Kronecker.readVector(in);
            System.out.println("¬ведите размерность матрицы I :");
            e = Kronecker.createEWithScanner(in);
            System.out.println("¬ведите число W :");
            int W = in.nextInt();
...
class Matrix {
    ...
    public static double[][] calculateInverseMatrix(Matrix matrix) {
        ...
        //создаем единичную матрицу дл€ формировани€ обратной
        for (int i = 0; i < n; i++) {
            test.inverse[i][i] = 1;
        }
        ...
        for (int i = 0; i < n; i++) {
            ...
            //занул€ем элементы под единицей
            for (int t = i + 1; t < n; t++) { //выбираем строку дл€ занулени€
                ...
                for (int k = 0; k < n; k++) { //занул€ем элемент и измен€ем строку
...
if(i==1){
Qshtrihii=sum(Q00,productMatrix(Q01,G0)); 
Qshtrihij=Q01;
}
else{
Qshtrihii=sum(Q1,productMatrix(Q2,G)); 
Qshtrihij=Q2;
}
...
return multipleVectorValue(calcP(F[i]),Math.e);
}
double L(){
double k=0;
for (int i=0;i<F.length;i++){
...
double probarrival(int i){
return VM(multipleVectorValue(calcP(F[i]),Math.e/calcLambda()),D1);
}
double time(int w, int m){
double w=0;
for (int i=0;i<F.length;i++){
w+=product2Vectors(prodVM(prodVM(calcP(i),kroneckerProductOfMatrixAndVector(I(m),createEAsVector(w)),inversionMatrix(S)),createEasVector(m));
}
w=sumV(w,multipleVectorValue(b1,L());
return w;
}